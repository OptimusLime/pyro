...
def empty_white(width, height):
    return torch.ones([4, width, height])

def image_to_pt(img_path):
    return surface_to_pt(img_to_surface(img_path))
...
def weighted_percent_same_bin(base_img, target_img, sobel_pt_img, weight):
    assert base_img.shape[1:] == target_img.shape[1:] and \
        base_img.shape[1:] == sobel_pt_img.shape[1:], \
        'weighted_percent_same_bin: image dimensions do not match!'
    
    width, height = base_img.shape[1:]

    # conver RGBA to grayscale
    this_gs, other_gs = rgba_to_grayscale(
        base_img).data, rgba_to_grayscale(target_img).data
    ...
    sim_tensor = torch.ones([1, width, height])
...
def surface_to_pt(img_surface):
    ...
    # make sure it's in
    return torch.from_numpy(np_surface[:3])
...
def load_target(target_file):
    ...
    target = image_to_pt(base_tgt_file + '.png')

    # add some properties
    # like starting position and direction
    target.start_pos = torch.from_numpy(coord_info[0]).float()
    target.start_dir = torch.from_numpy(coord_info[1]).float()
...
SOBEL_XY = torch.cat([SOBEL_X_FILTER, SOBEL_Y_FILTER],
                     dim=0).expand(2, 3, 3, 3)
BIAS = torch.zeros(SOBEL_XY.data.shape[0])
...
def make_gradient_weight_similarity_fct(edge_weight):
    ...
    def weighted_percent(base_img, tgt_img):
        # take our image objects and then create sobel
        sobel_pt_img = get_sobel(tgt_img)

        # return the weighted percent by similarity
        return weighted_percent_same_bin(
            base_img, tgt_img, sobel_pt_img, flat_weight)
...
def baseline_similarity(target_img, similarity_fct):
    ...
    white_img = empty_white(*target_img.shape[1:])
...
def normalized_similarity(img, target_img, similarity_fct):
    ...
    # compare render to target image via similarity function
    sim_comparison = similarity_fct(img, target_img)

    # haven't calculated baseline? do it here.
    if not hasattr(target_img, "baseline"):
        setattr(target_img, "baseline", 
                baseline_similarity(target_img, similarity_fct))
...
def render_branch(context, start_pos, end_pos, width):
    ...
    context.move_to(*start_pos.cpu().numpy())
    context.line_to(*end_pos.cpu().numpy())
    context.stroke()
    context.close_path()


# https://github.com/probmods/webppl/blob/gh-pages-vinesDemoFreeSketch/demos/vines/js/render.js#L304
def render_leaf(context, center, angle, scale):
    ...
    context.translate(*center.cpu().numpy())
...
def render_flower(context, center, radius):
    x,y = center.cpu().numpy()
...
# // Render update
# most logic inspired by this function:
...
def render_vines(
    ...
    vwidth = start_viewport["xmax"] - start_viewport["xmin"]
    vheight = start_viewport["ymax"] - start_viewport["ymin"]
    context.scale(img_size[0]/vwidth, img_size[1]/vheight)
    context.translate(-start_viewport["xmin"], -start_viewport["ymin"])

    # loop over the vine edges, draw the branches
    for (in_node, out_node) in vine_graph.edges():
        # draw each branch (should be in z-order)
        render_branch(
            context, 
            in_node.pos, out_node.pos, 
            branch_width)
    
    # loop over the vine nodes to draw individual
    # leafs or flowers 
    # (called after branch rendering is important for z dim)
    for node in vine_graph.nodes():
        if node.is_end:
            render_leaf(
                context,
                node.pos,
                np.pi/4,
                [2*leaf_radius, leaf_radius]
            )
        # some nodes have leafs AND flowers
        # draw the flower if necessary
        if node.is_flower:
            render_flower(
                context, 
                node.pos,
    ...
    # TODO: Adjust viewport? Seems to happen in webppl demo

    # return to our tensor
    return surface_to_pt(img_surface)
